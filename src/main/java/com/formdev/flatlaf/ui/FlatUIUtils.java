...
public class FlatUIUtils {
    ...
    public static void paintOutline(Graphics2D g, float x, float y, float w2, float h, float lineWidth, float arc) {
        FlatUIUtils.paintOutline(g, x, y, w2, h, lineWidth, arc, arc - lineWidth * 2.0f);
}
    public static void paintOutline(Graphics2D g, float x, float y, float w2, float h, float lineWidth, float arc, float innerArc) {
        if (lineWidth == 0.0f || w2 <= 0.0f || h <= 0.0f) {
            return;
}
        float t2 = lineWidth;
        float t2x = t2 * 2.0f;
        Path2D.Float border = new Path2D.Float(0);
        border.append(FlatUIUtils.createComponentRectangle(x, y, w2, h, arc), false);
        border.append(FlatUIUtils.createComponentRectangle(x + t2, y + t2, w2 - t2x, h - t2x, innerArc), false);
        g.fill(border);
}
    public static Shape createComponentRectangle(float x, float y, float w2, float h, float arc) {
        if (arc <= 0.0f) {
            return new Rectangle2D.Float(x, y, w2, h);
}
        if (w2 == h && arc >= w2) {
            return new Ellipse2D.Float(x, y, w2, h);
}
        arc = Math.min(arc, Math.min(w2, h));
        return new RoundRectangle2D.Float(x, y, w2, h, arc, arc);
}
    /*
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    static void paintFilledRectangle(Graphics g, Color color, float x, float y, float w2, float h) {
        Graphics2D g2 = (Graphics2D)g.create();
        try {
            FlatUIUtils.setRenderingHints(g2);
            g2.setColor(color);
            g2.fill(new Rectangle2D.Float(x, y, w2, h));
}
        finally {
            g2.dispose();
}
    ...
    public static Shape createArrowShape(int direction, boolean chevron, float w2, float h) {
        switch (direction) {
            case 1: {
                return FlatUIUtils.createPath(!chevron, 0.0, h, w2 / 2.0f, 0.0, w2, h);
}
            case 5: {
                return FlatUIUtils.createPath(!chevron, 0.0, 0.0, w2 / 2.0f, h, w2, 0.0);
}
            case 7: {
                return FlatUIUtils.createPath(!chevron, w2, 0.0, 0.0, h / 2.0f, w2, h);
}
            case 3: {
                return FlatUIUtils.createPath(!chevron, 0.0, 0.0, w2, h / 2.0f, 0.0, h);
}
}
