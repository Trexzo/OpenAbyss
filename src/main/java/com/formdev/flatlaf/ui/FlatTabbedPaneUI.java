...
implements FlatStylingSupport.StyleableUI {
    ...
    private void repaintTab(int tabIndex) {
        ...
        Rectangle r2 = this.getTabBounds(this.tabPane, tabIndex);
        if (r2 == null) {
            return;
}
    ...
    protected int calculateTabWidth(int tabPlacement, int tabIndex, FontMetrics metrics) {
        ...
        if (tabWidthMode == 1 && this.isHorizontalTabPlacement() && !this.inCalculateEqual) {
            ...
            try {
                int n2 = this.calculateMaxTabWidth(tabPlacement);
                return n2;
}
            finally {
    ...
    protected void paintTab(Graphics g, int tabPlacement, Rectangle[] rects, int tabIndex, Rectangle iconRect, Rectangle textRect) {
        ...
        int x = tabRect.x;
        int y = tabRect.y;
        int w2 = tabRect.width;
        int h = tabRect.height;
        boolean bl = isSelected = tabIndex == this.tabPane.getSelectedIndex();
        if (this.tabsOpaque || this.tabPane.isOpaque()) {
            this.paintTabBackground(g, tabPlacement, tabIndex, x, y, w2, h, isSelected);
}
        this.paintTabBorder(g, tabPlacement, tabIndex, x, y, w2, h, isSelected);
        if (this.isTabClosable(tabIndex)) {
            this.paintTabCloseButton(g, tabIndex, x, y, w2, h);
}
        if (isSelected) {
            this.paintTabSelection(g, tabPlacement, tabIndex, x, y, w2, h);
}
        ...
        if (this.tabViewport != null && (tabPlacement == 1 || tabPlacement == 3)) {
            ...
            if (!viewRect.contains(textRect)) {
                Rectangle r2 = viewRect.intersection(textRect);
                if (r2.x > viewRect.x) {
                    clippedTitle = JavaCompatibility.getClippedString(null, metrics, title, r2.width);
}
}
}
        if (!isCompact) {
    ...
    @Override
    protected void paintTabBackground(Graphics g, int tabPlacement, int tabIndex, int x, int y, int w2, int h, boolean isSelected) {
        Color background;
        boolean isCard;
        boolean bl = isCard = this.getTabType() == 1;
        if (!isCard && this.tabArc > 0 || isCard && this.cardTabArc > 0 || !isCard && this.selectedInsets != null && (this.selectedInsets.top != 0 || this.selectedInsets.left != 0 || this.selectedInsets.bottom != 0 || this.selectedInsets.right != 0)) {
            background = this.tabPane.getBackgroundAt(tabIndex);
        ...
        if (!isCard && this.selectedInsets != null) {
            ...
            w2 -= UIScale.scale(insets.left + insets.right);
            h -= UIScale.scale(insets.top + insets.bottom);
}
        background = this.getTabBackground(tabPlacement, tabIndex, isSelected);
        g.setColor(FlatUIUtils.deriveColor(background, this.tabPane.getBackground()));
        if (!isCard && this.tabArc > 0) {
            float arc = UIScale.scale((float)this.tabArc) / 2.0f;
            FlatUIUtils.paintSelection((Graphics2D)g, x, y, w2, h, null, arc, arc, arc, arc, 0);
        } else if (isCard && this.cardTabArc > 0) {
            ((Graphics2D)g).fill(this.createCardTabOuterPath(tabPlacement, x, y, w2, h));
        } else {
            g.fillRect(x, y, w2, h);
}
}
    ...
    @Override
    protected void paintTabBorder(Graphics g, int tabPlacement, int tabIndex, int x, int y, int w2, int h, boolean isSelected) {
        if (FlatClientProperties.clientPropertyBoolean(this.tabPane, "JTabbedPane.showTabSeparators", this.showTabSeparators) && !this.isLastInRun(tabIndex)) {
            if (this.getTabType() == 1) {
                ...
                if (tabIndex != selectedIndex - 1 && tabIndex != selectedIndex) {
                    this.paintTabSeparator(g, tabPlacement, x, y, w2, h);
}
            } else {
                this.paintTabSeparator(g, tabPlacement, x, y, w2, h);
}
        ...
        if (isSelected && this.getTabType() == 1) {
            this.paintCardTabBorder(g, tabPlacement, tabIndex, x, y, w2, h);
}
}
    protected void paintCardTabBorder(Graphics g, int tabPlacement, int tabIndex, int x, int y, int w2, int h) {
        Graphics2D g2 = (Graphics2D)g;
        Path2D.Float path = new Path2D.Float(0);
        path.append(this.createCardTabOuterPath(tabPlacement, x, y, w2, h), false);
        path.append(this.createCardTabInnerPath(tabPlacement, x, y, w2, h), false);
        g.setColor(this.tabSeparatorColor != null ? this.tabSeparatorColor : this.contentAreaColor);
        g2.fill(path);
}
    protected Shape createCardTabOuterPath(int tabPlacement, int x, int y, int w2, int h) {
        float arc = UIScale.scale((float)this.cardTabArc) / 2.0f;
        switch (tabPlacement) {
            default: {
                return FlatUIUtils.createRoundRectanglePath(x, y, w2, h, arc, arc, 0.0f, 0.0f);
}
            case 3: {
                return FlatUIUtils.createRoundRectanglePath(x, y, w2, h, 0.0f, 0.0f, arc, arc);
}
            case 2: {
                return FlatUIUtils.createRoundRectanglePath(x, y, w2, h, arc, 0.0f, arc, 0.0f);
}
            case 4: 
}
        return FlatUIUtils.createRoundRectanglePath(x, y, w2, h, 0.0f, arc, 0.0f, arc);
}
    protected Shape createCardTabInnerPath(int tabPlacement, int x, int y, int w2, int h) {
        float bw = UIScale.scale((float)this.contentSeparatorHeight);
        float arc = UIScale.scale((float)this.cardTabArc) / 2.0f - bw;
        switch (tabPlacement) {
            default: {
                return FlatUIUtils.createRoundRectanglePath((float)x + bw, (float)y + bw, (float)w2 - bw * 2.0f, (float)h - bw, arc, arc, 0.0f, 0.0f);
}
            case 3: {
                return FlatUIUtils.createRoundRectanglePath((float)x + bw, y, (float)w2 - bw * 2.0f, (float)h - bw, 0.0f, 0.0f, arc, arc);
}
            case 2: {
                return FlatUIUtils.createRoundRectanglePath((float)x + bw, (float)y + bw, (float)w2 - bw, (float)h - bw * 2.0f, arc, 0.0f, arc, 0.0f);
}
            case 4: 
}
        return FlatUIUtils.createRoundRectanglePath(x, (float)y + bw, (float)w2 - bw, (float)h - bw * 2.0f, 0.0f, arc, 0.0f, arc);
}
    protected void paintTabCloseButton(Graphics g, int tabIndex, int x, int y, int w2, int h) {
        ...
        Rectangle tabCloseRect = this.getTabCloseBounds(tabIndex, x, y, w2, h, this.calcRect);
        this.closeIcon.paintIcon(this.tabCloseButton, g, tabCloseRect.x, tabCloseRect.y);
}
    protected void paintTabSeparator(Graphics g, int tabPlacement, int x, int y, int w2, int h) {
        ...
        if (tabPlacement == 2 || tabPlacement == 4) {
            ((Graphics2D)g).fill(new Rectangle2D.Float((float)x + offset, (float)(y + h) - sepWidth, (float)w2 - offset * 2.0f, sepWidth));
        } else if (this.isLeftToRight()) {
            ((Graphics2D)g).fill(new Rectangle2D.Float((float)(x + w2) - sepWidth, (float)y + offset, sepWidth, (float)h - offset * 2.0f));
        } else {
            ((Graphics2D)g).fill(new Rectangle2D.Float(x, (float)y + offset, sepWidth, (float)h - offset * 2.0f));
}
}
    protected void paintTabSelection(Graphics g, int tabPlacement, int tabIndex, int x, int y, int w2, int h) {
        boolean atBottom;
        ...
        switch (tabPlacement) {
            ...
            case 2: {
                sx = atBottom ? x + w2 + contentInsets.left - tabSelectionHeight : x;
                sw = tabSelectionHeight;
                break;
}
            case 4: {
                sx = atBottom ? x - contentInsets.right : x + w2 - tabSelectionHeight;
                sw = tabSelectionHeight;
}
}
        ...
        if (arc <= 0.0f) {
            ...
            Area area = new Area(this.createCardTabOuterPath(tabPlacement, x, y, w2, h));
    ...
    protected void paintContentBorder(Graphics g, int tabPlacement, int selectedIndex) {
        ...
        int x = insets.left;
        int y = insets.top;
        int w2 = this.tabPane.getWidth() - insets.right - insets.left;
        ...
        Path2D.Float path = new Path2D.Float(0);
        path.append(new Rectangle2D.Float(x, y, w2, h), false);
        path.append(new Rectangle2D.Float((float)x + (float)ci.left / 100.0f, (float)y + (float)ci.top / 100.0f, (float)w2 - (float)ci.left / 100.0f - (float)ci.right / 100.0f, (float)h - (float)ci.top / 100.0f - (float)ci.bottom / 100.0f), false);
        if (this.getTabType() == 1) {
            ...
            if (this.isHorizontalTabPlacement()) {
                ...
                float x2 = tabPlacement == 2 ? (float)x : (float)(x + w2) - csh;
    ...
    protected Rectangle getTabCloseBounds(int tabIndex, int x, int y, int w2, int h, Rectangle dest) {
        ...
        dest.x = this.isLeftToRight() ? x + w2 - tabInsets.right / 3 * 2 - iconWidth : x + tabInsets.left / 3 * 2;
    ...
    implements LayoutManager {
        ...
        public void layoutContainer(Container parent) {
            ...
            if (tabPlacement == 1 || tabPlacement == 3) {
                ...
                int n2 = totalTabWidth = FlatTabbedPaneUI.this.rects.length > 0 ? FlatTabbedPaneUI.this.rectsTotalWidth(leftToRight) : 0;
                ...
                int n3 = leftWidth = leftToRight ? leadingWidth : trailingWidth;
                ...
                int n4 = rightWidth = leftToRight ? trailingWidth : leadingWidth;
                ...
                if (FlatTabbedPaneUI.this.rects.length > 0) {
                    ...
                    if (w2 < totalTabWidth) {
                        ...
                        if (useMoreTabsButton) {
                            ...
                            FlatTabbedPaneUI.this.moreTabsButton.setBounds(leftToRight ? x + w2 - buttonWidth : x, ty, buttonWidth, th);
                        ...
                        if (useScrollButtons) {
                            if (!hideDisabledScrollButtons || forwardButton.isEnabled()) {
                                ...
                                forwardButton.setBounds(leftToRight ? x + w2 - buttonWidth : x, ty, buttonWidth, th);
                                x += leftToRight ? 0 : buttonWidth;
                                w2 -= buttonWidth;
                            ...
                            if (!hideDisabledScrollButtons || backwardButton.isEnabled()) {
                                ...
                                if (trailingScrollButtons) {
                                    backwardButton.setBounds(leftToRight ? x + w2 - buttonWidth : x, ty, buttonWidth, th);
                                    x += leftToRight ? 0 : buttonWidth;
                                } else {
                                    backwardButton.setBounds(leftToRight ? x : x + w2 - buttonWidth, ty, buttonWidth, th);
                    ...
                    FlatTabbedPaneUI.this.tabViewport.setBounds(x, ty, w2, th);
                    if (!leftToRight) {
                ...
                int n5 = totalTabHeight = FlatTabbedPaneUI.this.rects.length > 0 ? FlatTabbedPaneUI.this.rectsTotalHeight() : 0;
