...
implements FlatStylingSupport.StyleableUI {
    ...
    protected void paintButtonGroup(Graphics g) {
        ...
        for (int i = 0; !(i >= n2 || (b = componentArray[i]) instanceof AbstractButton && ((AbstractButton)b).getModel().isRollover() && (group = this.getButtonGroup((AbstractButton)b)) != null); ++i) {
}
        if (group == null) {
            return;
}
        ...
        for (Rectangle r3 : rects) {
            ...
            if (horizontal ? gr.x + gr.width + maxSepWidth >= r3.x : gr.y + gr.height + maxSepWidth >= r3.y) {
                gr = gr.union(r3);
                continue;
}
