...
extends JComponent {
    ...
    public FlatTitlePane(JRootPane rootPane) {
        ...
        Window w2 = SwingUtilities.getWindowAncestor(rootPane);
        String defaultWindowStyle = w2 != null && w2.getType() == Window.Type.UTILITY ? "small" : null;
    ...
    protected void addSubComponents() {
        ...
        this.setLayout(new BorderLayout(){
            ...
            public void layoutContainer(Container target) {
                ...
                int titleWidth = w2 - leftWidth - buttonsWidth;
                ...
                if (titleWidth < minTitleWidth) {
                    ...
                    titleWidth = w2 - leftWidth - buttonsWidth;
}
                if (titleWidth < minTitleWidth) {
    ...
    public void paint(Graphics g) {
        ...
        if (this.debugHitTestSpots != null) {
            for (Rectangle r2 : this.debugHitTestSpots) {
                this.paintRect(g, Color.red, r2);
}
}
    ...
    private void paintRect(Graphics g, Color color, Rectangle r2) {
        if (r2 == null) {
            return;
}
        g.setColor(color);
        Point offset = SwingUtilities.convertPoint(this, 0, 0, this.window);
        g.drawRect(r2.x - offset.x, r2.y - offset.y, r2.width - 1, r2.height - 1);
}
    @Override
    ...
    protected void updateNativeTitleBarHeightAndHitTestSpots() {
        ...
        Rectangle r2 = this.getNativeHitTestSpot(this.buttonPanel);
        if (r2 != null) {
            hitTestSpots.add(r2);
}
        if (this.hasVisibleEmbeddedMenuBar(menuBar = this.rootPane.getJMenuBar()) && (r2 = this.getNativeHitTestSpot(menuBar)) != null) {
            ...
            for (int i = count - 1; i >= 0; --i) {
                ...
                if (this.getComponentOrientation().isLeftToRight()) {
                    r22 = new Rectangle(x2, r2.y, r2.x + r2.width - x2, r2.height);
                    r2.width = glueLocation.x - r2.x;
                } else {
                    r22 = new Rectangle(r2.x, r2.y, glueLocation.x - r2.x, r2.height);
                    r2.width = r2.x + r2.width - x2;
                    r2.x = x2;
}
                if (r22.width <= 0) continue;
                hitTestSpots.add(r22);
}
            hitTestSpots.add(r2);
}
        for (Component c : this.rootPane.getLayeredPane().getComponents()) {
            Rectangle rectangle = r2 = c instanceof JInternalFrame ? this.getNativeHitTestSpot((JInternalFrame)c) : null;
            if (r2 == null) continue;
            hitTestSpots.add(r2);
}
        ...
        protected String layoutCL(JLabel label, FontMetrics fontMetrics, String text, Icon icon, Rectangle viewR, Rectangle iconR, Rectangle textR) {
            ...
            if (!(clippedText = SwingUtilities.layoutCompoundLabel(label, fontMetrics, text, icon, label.getVerticalAlignment(), label.getHorizontalAlignment(), label.getVerticalTextPosition(), label.getHorizontalTextPosition(), viewR, iconR, textR, iconTextGap)).equals(text)) {
                ...
                if (center) {
                    ...
                    int n2 = textR.x = leftToRight ? Math.min(viewR.x + leadingGap + iconWidthAndGap, viewR.x + viewR.width - textR.width) : Math.max(viewR.x + viewR.width - leadingGap - iconWidthAndGap - textR.width, viewR.x);
}
}
            if (icon != null) {
