...
extends FlatTextFieldUI {
    ...
    protected void paintIcons(Graphics g, Rectangle r2) {
        super.paintIcons(g, r2);
        if (this.isCapsLockVisible()) {
            this.paintCapsLock(g, r2);
}
}
    protected void paintCapsLock(Graphics g, Rectangle r2) {
        JTextComponent c = this.getComponent();
        int x = c.getComponentOrientation().isLeftToRight() ? r2.x + r2.width - this.capsLockIcon.getIconWidth() : r2.x;
        int y = r2.y + Math.round((float)(r2.height - this.capsLockIcon.getIconHeight()) / 2.0f);
