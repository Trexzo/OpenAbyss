...
public class HSLColor {
    ...
    public static float[] fromRGB(Color color) {
        ...
        float g = rgb[1];
        float b = rgb[2];
        float min = Math.min(r2, Math.min(g, b));
        float max = Math.max(r2, Math.max(g, b));
        float h = 0.0f;
        if (max == min) {
            h = 0.0f;
        } else if (max == r2) {
            ...
            h = 60.0f * (b - r2) / (max - min) + 120.0f;
        } else if (max == b) {
            h = 60.0f * (r2 - g) / (max - min) + 240.0f;
}
